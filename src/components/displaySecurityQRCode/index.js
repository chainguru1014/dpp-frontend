...
// For production: http://localhost:3000 (or your VPS IP/domain)
// Default to VPS URL for production use
const SECURITY_BASE_URL = process.env.REACT_APP_SECURITY_BASE_URL || process.env.REACT_APP_WEB_BASE_URL || 'http://localhost:3000';
...
const SecurityQRCode = ({data, identifer}) => {
    ...
    useEffect(() => {
        (async () => {
            // Generate QR code with security URL format: http://localhost:3000/product/{encryptedKey}
